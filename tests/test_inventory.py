...
def test_runtime_tools_equal_allowlist(tmp_path, monkeypatch):
    inv = _fresh_inventory(tmp_path, monkeypatch)
    names = inv.all_tool_names()
    assert {"exists", "verify_claim"}.issubset(names)
    # Le toolset runtime = EXACTEMENT l'allow-list (OUTILS du LLM), sans parsing
    # du system_prompt qui pourrait rajouter des outils.
    spec = inv.save_agent_spec({
        "title": "Cite", "template": "libre", "writes": False,
        "allowed_tools": ["exists", "verify_claim"],
        # le system_prompt cite plein d'autres outils : ils NE doivent PAS être
        # ajoutés.
        "system_prompt": "1. get_synonyms(x)\n2. detect_gaps(y)\n3. disambiguate(z)",
    })
    remaining = names - inv.exclude_tools_for_spec(spec)
