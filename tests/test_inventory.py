...
def test_output_ext_sanitized(tmp_path, monkeypatch):
    inv = _fresh_inventory(tmp_path, monkeypatch)
    spec = inv.save_agent_spec({
        "title": "X", "template": "audit", "output_ext": "exe",  # interdit
    })
    # retombe sur le défaut du template (.audit), pas '.exe'
    assert spec["output_ext"] == ".audit"
