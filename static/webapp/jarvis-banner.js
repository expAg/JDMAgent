(function() {
  ...
  function _mix(h, t, to) {
    const a = _hx(h);
    return `rgb(${Math.round(a[0] + (to[0] - a[0]) * t)},${Math.round(a[1] + (to[1] - a[1]) * t)},${Math.round(a[2] + (to[2] - a[2]) * t)})`;
  }
  ...
  const TRIPLETS = [
    ["chat", "r_isa", "animal"],
    ["pomme", "r_isa", "fruit"],
    ["soleil", "r_carac", "chaud"],
    ["oiseau", "r_agent", "voler"],
    ["mer", "r_carac", "salé"],
    ["rose", "r_color", "rouge"],
    ["abeille", "r_agent", "butiner"],
    ["poisson", "r_lieu", "eau"],
    ["paris", "r_isa", "ville"]
  ];
  ...
  function gChain(accent) {
    ...
    let x = 2;
    c.n.forEach((w) => {
      const ww = w.length * 6.2 + 16;
      pos.push({ x, w: ww });
      x += ww + GAP;
    });
    const W = x - GAP + 4;
    const SC = 0.7;
    let s = `<svg width="${Math.round(W * SC)}" height="${Math.round(h * SC)}" viewBox="0 0 ${Math.round(W)} ${h}">`;
  ...
  function gStar(accent) {
    ...
    const SC = 0.7;
    let s = `<svg width="${Math.round(W * SC)}" height="${Math.round(H * SC)}" viewBox="0 0 ${W} ${H}">`;
    st.l.forEach((leaf, i) => {
  ...
  function pickThought(accent) {
    const x = Math.random();
    if (x < 0.26) return gChain(accent);
    if (x < 0.5) return gMesh();
    if (x < 0.66) return gStar(accent);
    if (x < 0.8) return gTriplet(accent);
    if (x < 0.9) return { expr: "eureka", kind: "idea", html: bulbSVG(accent) + `<span class="jb-spark" style="color:${accent}">!</span>` };
  ...
  function Robot({ mode, dark, speedPct, sizePx, expressivity, freq, manualExpr, frozen, chatBusy }) {
    ...
    useEffect(() => {
      ...
      const onMove = (e) => {
        ...
        A.mouse.x = e.clientX - b.left;
        A.mouse.y = e.clientY - b.top;
        A.mouse.on = true;
      };
      const onLeave = () => {
        A.mouse.on = false;
      ...
      const measureBound = () => {
        try {
          ...
          boundRef.current = {
            lb: Math.max(36, titleRight + 18),
            // limite gauche = le « d » de bord (extent réel du texte)
            lb2: Math.max(36, descRight + 18),
            // au-delà : pleine hauteur (sous la description)
            upMaxY: titleBottom + 6
            // bande haute = à côté du titre, au-dessus du texte
          };
        } catch (e) {
        }
      };
      ...
      const reHome = () => {
        if (stateRef.current.mode !== "auto" && !A.mouse.on) homePos();
      };
      const bt1 = setTimeout(() => {
        measureBound();
        reHome();
      }, 300), bt2 = setTimeout(() => {
        measureBound();
      ...
      const setT = (k, t) => {
        if (r.current[k]) r.current[k].setAttribute("transform", t);
      };
      let raf;
      const tick = (now) => {
        try {
          const dt = Math.min(0.045, (now - A.last) / 1e3);
          A.last = now;
          A.t += dt;
          const S = stateRef.current, auto = S.mode === "auto";
          if (S.frozen && !A._frozen) {
            A._frozen = true;
            measureBound();
            frozenPos();
          }
          ...
          if (S.frozen) {
            ...
            if (A.t > A.nextWp) {
              A.tx = lb + Math.random() * (W - mx - lb);
              A.ty = myTop + Math.random() * (yMaxFor(A.tx) - myTop);
              A.nextWp = A.t + 0.5 + Math.random() * 1.4;
            }
          } else if (A.mouse.on) {
            A.tx = clamp(A.mouse.x, lb, W - mx);
            A.ty = clamp(A.mouse.y, myTop, yMaxFor(A.tx));
          } else {
            A.tx = A.x;
            A.ty = A.y;
          }
          ...
          let desVX = 0, desVY = 0;
          if (dist > 18) {
            const sp = dist < 110 ? maxV * (dist / 110) : maxV;
            desVX = dx / dist * sp;
            desVY = dy / dist * sp;
          }
          A.vx += (desVX - A.vx) * (1 - Math.exp(-dt * 9));
          A.vy += (desVY - A.vy) * (1 - Math.exp(-dt * 9));
          A.x += A.vx * dt;
          A.y += A.vy * dt;
          A.x = clamp(A.x, lb, W - mx);
          A.y = clamp(A.y, myTop, yMaxFor(A.x));
          ...
          A.pup.y += (clamp(look.y / ln, -1, 1) * 2 - A.pup.y) * (1 - Math.exp(-dt * 10));
          let eyeS = 1;
          if (!A.blink.on && A.t > A.nextBlink) {
            A.blink.on = true;
            A.blink.s = A.t;
          }
          if (A.blink.on) {
            const p = A.t - A.blink.s;
            if (p < 0.06) eyeS = 1 - p / 0.06;
            else if (p < 0.12) eyeS = (p - 0.06) / 0.06;
            else {
              eyeS = 1;
              A.blink.on = false;
              A.nextBlink = A.t + 1.8 + Math.random() * 3.2;
            }
          }
          let armLa, armRa;
          if (running) {
            ...
            if (S.expr === "eureka") {
              ...
              armRa = -50;
              armLa = sway * 0.4;
            } else if (S.expr === "curious") {
              armLa = 22;
              armRa = -22;
            }
            if (A.t > A.waveAt && S.expr !== "eureka") {
              A.waveOn = true;
              A.waveS = A.t;
              A.waveAt = A.t + 5 + Math.random() * 5;
            }
            if (A.waveOn) {
              const p = A.t - A.waveS;
              if (p < 1.2) armRa = -48 + Math.sin(p * 19) * 24 * xv;
              else A.waveOn = false;
            }
          }
          ...
          for (const B2 of bubblesRef.current) placeTail(B2, A.x, rty);
          ...
          if (running && speed > maxV * 0.45 && puffCd <= 0) {
            const p = puffs[puffIx % puffs.length];
            puffIx++;
            puffCd = 0.09;
            p.life = 1;
            p.max = 0.5 + Math.random() * 0.2;
            p.el.style.left = A.x - A.dir * sw * 0.12 - 5 + "px";
            p.el.style.top = A.y - 5 + "px";
          }
          puffs.forEach((p) => {
            if (p.life > 0) {
              p.life -= dt / p.max;
              ...
              p.el.style.transform = `translate(${((1 - k) * -9).toFixed(1)}px, ${((1 - k) * -7).toFixed(1)}px) scale(${(0.5 + (1 - k) * 1).toFixed(2)})`;
            } else if (p.el.style.opacity !== "0") p.el.style.opacity = "0";
          });
          raf = requestAnimationFrame(tick);
        } catch (err) {
          console.error("[jb-robot]", err);
        }
      };
      raf = requestAnimationFrame(tick);
      return () => {
        ...
        window.removeEventListener("resize", onResizeHome);
        clearTimeout(bt1);
        clearTimeout(bt2);
    ...
    useEffect(() => {
      ...
      const cycle = () => {
        ...
        if (field) {
          ...
          bubblesRef.current = [];
          const d = document.createElement("div");
          d.className = "jb-thought jb-thought--" + th.kind;
          d.innerHTML = th.html;
    ...
    const glow = (cx) => /* @__PURE__ */ React.createElement("ellipse", { cx, cy: "45", rx: "12", ry: "12", fill: "url(#jbeyeGlow)" });
    const glassEye = (cx) => /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("rect", { x: cx - 5.5, y: "38", width: "11", height: "15", rx: "5.5", fill: "url(#jbeyeFill)" }), /* @__PURE__ */ React.createElement("ellipse", { cx: cx - 1.6, cy: "42", rx: "3.4", ry: "2.3", fill: "#fff", opacity: "0.55" }));
    const glassCircle = (cx, rr) => /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("circle", { cx, cy: "45", r: rr, fill: "url(#jbeyeFill)" }), /* @__PURE__ */ React.createElement("ellipse", { cx: cx - rr * 0.32, cy: 45 - rr * 0.34, rx: rr * 0.42, ry: rr * 0.28, fill: "#fff", opacity: "0.5" }));
    const star = (cx, cy, s) => /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("path", { d: `M${cx} ${cy - s} L${cx + s * 0.28} ${cy - s * 0.28} L${cx + s} ${cy} L${cx + s * 0.28} ${cy + s * 0.28} L${cx} ${cy + s} L${cx - s * 0.28} ${cy + s * 0.28} L${cx - s} ${cy} L${cx - s * 0.28} ${cy - s * 0.28} Z`, fill: "url(#jbeyeFill)", stroke: accent, strokeWidth: "0.6" }), /* @__PURE__ */ React.createElement("circle", { cx: cx - s * 0.18, cy: cy - s * 0.18, r: "1.5", fill: "#fff", opacity: "0.85" }));
    const closedEye = (cx) => /* @__PURE__ */ React.createElement("path", { d: `M${cx - 7} 44 q7 7 14 0`, fill: "none", stroke: accent, strokeWidth: "3.4", strokeLinecap: "round" });
    const pupilDots = (xs) => /* @__PURE__ */ React.createElement("g", { ref: ref("pupils") }, xs.map((cx, i) => /* @__PURE__ */ React.createElement("circle", { key: i, cx, cy: "47.5", r: "1.9", fill: "#fff", opacity: "0.9" })));
    const babyEyes = /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("ellipse", { cx: "50", cy: "46", rx: "9", ry: "10.5", fill: "#fdfbf7" }), /* @__PURE__ */ React.createElement("ellipse", { cx: "70", cy: "46", rx: "9", ry: "10.5", fill: "#fdfbf7" }), /* @__PURE__ */ React.createElement("g", { ref: ref("pupils") }, /* @__PURE__ */ React.createElement("circle", { cx: "50", cy: "47", r: "6", fill: "url(#jbeyeFill)" }), /* @__PURE__ */ React.createElement("circle", { cx: "70", cy: "47", r: "6", fill: "url(#jbeyeFill)" }), /* @__PURE__ */ React.createElement("circle", { cx: "50", cy: "48", r: "3", fill: "#241c2e" }), /* @__PURE__ */ React.createElement("circle", { cx: "70", cy: "48", r: "3", fill: "#241c2e" }), /* @__PURE__ */ React.createElement("circle", { cx: "47.8", cy: "45", r: "2.5", fill: "#fff" }), /* @__PURE__ */ React.createElement("circle", { cx: "67.8", cy: "45", r: "2.5", fill: "#fff" }), /* @__PURE__ */ React.createElement("circle", { cx: "52", cy: "49.5", r: "1.2", fill: "#fff", opacity: "0.85" }), /* @__PURE__ */ React.createElement("circle", { cx: "72", cy: "49.5", r: "1.2", fill: "#fff", opacity: "0.85" })));
    let eyesJSX;
    if (expr === "baby") eyesJSX = babyEyes;
    else if (expr === "starry" || expr === "eureka") eyesJSX = /* @__PURE__ */ React.createElement(React.Fragment, null, star(50, 44, 8.6), star(70, 44, 8.6));
    else if (expr === "thinking") eyesJSX = /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("path", { d: "M44 46 q6 -7 12 0", fill: "none", stroke: accent, strokeWidth: "3.6", strokeLinecap: "round" }), /* @__PURE__ */ React.createElement("path", { d: "M64 46 q6 -7 12 0", fill: "none", stroke: accent, strokeWidth: "3.6", strokeLinecap: "round" }));
    else if (expr === "curious") eyesJSX = /* @__PURE__ */ React.createElement(React.Fragment, null, glow(50), glow(70), glassCircle(50, 7.5), glassCircle(70, 7.5), pupilDots([50, 70]));
    else if (expr === "closed") eyesJSX = /* @__PURE__ */ React.createElement(React.Fragment, null, closedEye(50), closedEye(70));
    else if (expr === "wink") eyesJSX = /* @__PURE__ */ React.createElement(React.Fragment, null, closedEye(50), glow(70), glassEye(70), pupilDots([70]));
    else eyesJSX = /* @__PURE__ */ React.createElement(React.Fragment, null, glow(50), glow(70), glassEye(50), glassEye(70), pupilDots([50, 70]));
    let mouthJSX;
    if (expr === "baby") mouthJSX = /* @__PURE__ */ React.createElement("path", { d: "M55 56 q5 4.5 10 0", fill: "none", stroke: "#d05a86", strokeWidth: "2.2", strokeLinecap: "round" });
    else if (expr === "eureka") mouthJSX = /* @__PURE__ */ React.createElement("path", { d: "M53 54 q7 7 14 0 q-7 2 -14 0 Z", fill: accent, opacity: "0.9" });
    else if (expr === "thinking") mouthJSX = /* @__PURE__ */ React.createElement("circle", { cx: "60", cy: "56", r: "2.3", fill: "none", stroke: accent, strokeWidth: "1.8", opacity: "0.7" });
    else if (expr === "curious") mouthJSX = /* @__PURE__ */ React.createElement("path", { d: "M55 56 q2.5 2.5 5 0 q2.5 -2.5 5 0", fill: "none", stroke: accent, strokeWidth: "1.8", strokeLinecap: "round", opacity: "0.7" });
    else mouthJSX = /* @__PURE__ */ React.createElement("path", { d: "M53 55 q7 5 14 0", fill: "none", stroke: accent, strokeWidth: "2", strokeLinecap: "round", opacity: "0.8" });
    return /* @__PURE__ */ React.createElement("div", { className: "jb-robot-field", ref: wrapRef }, /* @__PURE__ */ React.createElement("div", { className: "jb-robot", ref: robotRef }, /* @__PURE__ */ React.createElement("svg", { viewBox: "0 0 120 150", width: "100%", style: { overflow: "visible", display: "block" } }, /* @__PURE__ */ React.createElement("defs", null, /* @__PURE__ */ React.createElement("linearGradient", { id: "jbbody", x1: "0", y1: "0", x2: "0", y2: "1" }, /* @__PURE__ */ React.createElement("stop", { offset: "0", stopColor: "#ffffff" }), /* @__PURE__ */ React.createElement("stop", { offset: "0.55", stopColor: "#f3eee2" }), /* @__PURE__ */ React.createElement("stop", { offset: "1", stopColor: "#dcd4c4" })), /* @__PURE__ */ React.createElement("linearGradient", { id: "jbmetal", x1: "0", y1: "0", x2: "0", y2: "1" }, /* @__PURE__ */ React.createElement("stop", { offset: "0", stopColor: "#eceadf" }), /* @__PURE__ */ React.createElement("stop", { offset: "1", stopColor: "#c4bcab" })), /* @__PURE__ */ React.createElement("linearGradient", { id: "jbvisor", x1: "0", y1: "0", x2: "0", y2: "1" }, /* @__PURE__ */ React.createElement("stop", { offset: "0", stopColor: "#33353f" }), /* @__PURE__ */ React.createElement("stop", { offset: "0.5", stopColor: "#1a1b22" }), /* @__PURE__ */ React.createElement("stop", { offset: "1", stopColor: "#0b0c10" })), /* @__PURE__ */ React.createElement("linearGradient", { id: "jbjdm", x1: "0", y1: "0", x2: "1", y2: "0" }, /* @__PURE__ */ React.createElement("stop", { offset: "0", stopColor: "#E63B7A" }), /* @__PURE__ */ React.createElement("stop", { offset: "0.25", stopColor: "#F5C518" }), /* @__PURE__ */ React.createElement("stop", { offset: "0.5", stopColor: "#5FB94A" }), /* @__PURE__ */ React.createElement("stop", { offset: "0.75", stopColor: "#2BB8D4" }), /* @__PURE__ */ React.createElement("stop", { offset: "1", stopColor: "#8A5CD4" })), /* @__PURE__ */ React.createElement("radialGradient", { id: "jbeyeGlow", cx: "50%", cy: "50%", r: "50%" }, /* @__PURE__ */ React.createElement("stop", { offset: "0", stopColor: accent, stopOpacity: "0.6" }), /* @__PURE__ */ React.createElement("stop", { offset: "1", stopColor: accent, stopOpacity: "0" })), /* @__PURE__ */ React.createElement("linearGradient", { id: "jbonesie", x1: "0", y1: "0", x2: "0", y2: "1" }, /* @__PURE__ */ React.createElement("stop", { offset: "0", stopColor: "#eaf7fa" }), /* @__PURE__ */ React.createElement("stop", { offset: "1", stopColor: "#bfe3ec" })), /* @__PURE__ */ React.createElement("linearGradient", { id: "jbeyeFill", x1: "0", y1: "0", x2: "0", y2: "1" }, /* @__PURE__ */ React.createElement("stop", { offset: "0", stopColor: lighten(accent, 0.6) }), /* @__PURE__ */ React.createElement("stop", { offset: "0.45", stopColor: accent }), /* @__PURE__ */ React.createElement("stop", { offset: "1", stopColor: darken(accent, 0.42) }))), /* @__PURE__ */ React.createElement("ellipse", { cx: "60", cy: "128", rx: "24", ry: "5", fill: "#000", opacity: dark ? 0.36 : 0.15 }), /* @__PURE__ */ React.createElement("g", { ref: ref("lean") }, /* @__PURE__ */ React.createElement("g", { ref: ref("face") }, /* @__PURE__ */ React.createElement("g", { ref: ref("bob") }, /* @__PURE__ */ React.createElement("g", { ref: ref("legL") }, /* @__PURE__ */ React.createElement("rect", { x: "47", y: "102", width: "11", height: "15", rx: "5.5", fill: "url(#jbmetal)" }), /* @__PURE__ */ React.createElement("ellipse", { cx: "52.5", cy: "115", rx: "6.5", ry: "2.4", fill: "#fff", opacity: "0.4" })), /* @__PURE__ */ React.createElement("g", { ref: ref("legR") }, /* @__PURE__ */ React.createElement("rect", { x: "62", y: "102", width: "11", height: "15", rx: "5.5", fill: "url(#jbmetal)" }), /* @__PURE__ */ React.createElement("ellipse", { cx: "67.5", cy: "115", rx: "6.5", ry: "2.4", fill: "#fff", opacity: "0.4" })), /* @__PURE__ */ React.createElement("g", { ref: ref("armL") }, /* @__PURE__ */ React.createElement("circle", { cx: "30", cy: "90", r: "7", fill: "url(#jbbody)", stroke: "rgba(40,32,22,0.10)", strokeWidth: "1.2" })), /* @__PURE__ */ React.createElement("g", { ref: ref("armR") }, /* @__PURE__ */ React.createElement("circle", { cx: "90", cy: "90", r: "7", fill: "url(#jbbody)", stroke: "rgba(40,32,22,0.10)", strokeWidth: "1.2" })), /* @__PURE__ */ React.createElement("rect", { x: "43", y: "72", width: "34", height: "34", rx: "17", fill: "url(#jbonesie)", stroke: "rgba(31,151,177,0.20)", strokeWidth: "1.4" }), /* @__PURE__ */ React.createElement("ellipse", { cx: "60", cy: "79", rx: "11", ry: "4.5", fill: "#ffffff", opacity: "0.4" }), /* @__PURE__ */ React.createElement("circle", { cx: "60", cy: "83", r: "1.9", fill: "#E63B7A" }), /* @__PURE__ */ React.createElement("circle", { cx: "60", cy: "90", r: "1.9", fill: "#F5C518" }), /* @__PURE__ */ React.createElement("circle", { cx: "60", cy: "97", r: "1.9", fill: "#5FB94A" }), /* @__PURE__ */ React.createElement("g", null, /* @__PURE__ */ React.createElement("rect", { x: "26", y: "12", width: "68", height: "62", rx: "24", fill: "url(#jbbody)", stroke: "rgba(40,32,22,0.12)", strokeWidth: "1.5" }), /* @__PURE__ */ React.createElement("ellipse", { cx: "60", cy: "22", rx: "24", ry: "6", fill: "#ffffff", opacity: "0.45" }), /* @__PURE__ */ React.createElement("rect", { x: "20", y: "38", width: "8", height: "18", rx: "4", fill: "url(#jbmetal)", stroke: "rgba(40,32,22,0.12)", strokeWidth: "1" }), /* @__PURE__ */ React.createElement("circle", { cx: "24", cy: "47", r: "2", fill: "#E63B7A" }), /* @__PURE__ */ React.createElement("rect", { x: "92", y: "38", width: "8", height: "18", rx: "4", fill: "url(#jbmetal)", stroke: "rgba(40,32,22,0.12)", strokeWidth: "1" }), /* @__PURE__ */ React.createElement("circle", { cx: "96", cy: "47", r: "2", fill: "#2BB8D4" }), /* @__PURE__ */ React.createElement("circle", { cx: "34", cy: "20", r: "1.4", fill: "rgba(40,32,22,0.25)" }), /* @__PURE__ */ React.createElement("circle", { cx: "86", cy: "20", r: "1.4", fill: "rgba(40,32,22,0.25)" }), /* @__PURE__ */ React.createElement("g", { ref: ref("ant") }, /* @__PURE__ */ React.createElement("line", { x1: "60", y1: "12", x2: "60", y2: "5", stroke: "#b8b0a0", strokeWidth: "2", strokeLinecap: "round" }), /* @__PURE__ */ React.createElement("circle", { cx: "60", cy: "3.5", r: "3.4", fill: accent }, mode === "auto" ? /* @__PURE__ */ React.createElement("animate", { attributeName: "fill", values: "#E63B7A;#F5C518;#5FB94A;#2BB8D4;#8A5CD4;#E63B7A", dur: "6s", repeatCount: "indefinite" }) : /* @__PURE__ */ React.createElement("animate", { attributeName: "r", values: "3.1;4;3.1", dur: "1.5s", repeatCount: "indefinite" }))), /* @__PURE__ */ React.createElement("text", { x: "54", y: "24.2", textAnchor: "middle", fontFamily: "'Lilita One',system-ui", fontSize: "7.6", fill: "#c83a73" }, "j"), /* @__PURE__ */ React.createElement("text", { x: "60", y: "24.2", textAnchor: "middle", fontFamily: "'Lilita One',system-ui", fontSize: "7.6", fill: "#4ea63c" }, "d"), /* @__PURE__ */ React.createElement("text", { x: "66", y: "24.2", textAnchor: "middle", fontFamily: "'Lilita One',system-ui", fontSize: "7.6", fill: "#1f97b1" }, "m"), /* @__PURE__ */ React.createElement("rect", { x: "31", y: "27", width: "58", height: "38", rx: "17", fill: "url(#jbjdm)", opacity: "0.95" }), /* @__PURE__ */ React.createElement("rect", { x: "33.5", y: "29.5", width: "53", height: "33", rx: "15", fill: "url(#jbvisor)" }), /* @__PURE__ */ React.createElement("rect", { x: "37", y: "31", width: "46", height: "9", rx: "4.5", fill: "#ffffff", opacity: "0.06" }), /* @__PURE__ */ React.createElement("circle", { cx: "40", cy: "57", r: "3", fill: "#F2A05A", opacity: "0.5" }), /* @__PURE__ */ React.createElement("circle", { cx: "80", cy: "57", r: "3", fill: "#F2A05A", opacity: "0.5" }), /* @__PURE__ */ React.createElement("g", { ref: ref("eyes") }, eyesJSX), mouthJSX)))))));
  }
  function MiniRobot({ dark, awake }) {
    const accent = "#2BD4C0";
    const cx = 40, cy = 39;
    return /* @__PURE__ */ React.createElement("svg", { className: "jb-mini-svg", viewBox: "0 0 80 74", width: "38", height: "35", style: { display: "block", overflow: "visible" } }, /* @__PURE__ */ React.createElement("defs", null, /* @__PURE__ */ React.createElement("linearGradient", { id: "jbmbody", x1: "0", y1: "0", x2: "0", y2: "1" }, /* @__PURE__ */ React.createElement("stop", { offset: "0", stopColor: "#ffffff" }), /* @__PURE__ */ React.createElement("stop", { offset: "0.55", stopColor: "#f3eee2" }), /* @__PURE__ */ React.createElement("stop", { offset: "1", stopColor: "#dcd4c4" })), /* @__PURE__ */ React.createElement("linearGradient", { id: "jbmjdm", x1: "0", y1: "0", x2: "1", y2: "0" }, /* @__PURE__ */ React.createElement("stop", { offset: "0", stopColor: "#E63B7A" }), /* @__PURE__ */ React.createElement("stop", { offset: "0.25", stopColor: "#F5C518" }), /* @__PURE__ */ React.createElement("stop", { offset: "0.5", stopColor: "#5FB94A" }), /* @__PURE__ */ React.createElement("stop", { offset: "0.75", stopColor: "#2BB8D4" }), /* @__PURE__ */ React.createElement("stop", { offset: "1", stopColor: "#8A5CD4" })), /* @__PURE__ */ React.createElement("linearGradient", { id: "jbmvisor", x1: "0", y1: "0", x2: "0", y2: "1" }, /* @__PURE__ */ React.createElement("stop", { offset: "0", stopColor: "#33353f" }), /* @__PURE__ */ React.createElement("stop", { offset: "0.5", stopColor: "#1a1b22" }), /* @__PURE__ */ React.createElement("stop", { offset: "1", stopColor: "#0b0c10" }))), /* @__PURE__ */ React.createElement("line", { x1: "40", y1: "14", x2: "40", y2: "6", stroke: "#b8b0a0", strokeWidth: "2", strokeLinecap: "round" }), /* @__PURE__ */ React.createElement("circle", { cx: "40", cy: "4.5", r: "3", fill: accent }, awake ? /* @__PURE__ */ React.createElement("animate", { attributeName: "opacity", values: "0.5;1;0.5", dur: "2.2s", repeatCount: "indefinite" }) : /* @__PURE__ */ React.createElement("animate", { attributeName: "opacity", values: "0.18;0.32;0.18", dur: "4s", repeatCount: "indefinite" })), /* @__PURE__ */ React.createElement("rect", { x: "14", y: "14", width: "52", height: "48", rx: "19", fill: "url(#jbmbody)", stroke: "rgba(40,32,22,0.14)", strokeWidth: "1.4" }), /* @__PURE__ */ React.createElement("rect", { x: "10", y: "32", width: "6", height: "14", rx: "3", fill: "#cfc8b8" }), /* @__PURE__ */ React.createElement("rect", { x: "64", y: "32", width: "6", height: "14", rx: "3", fill: "#cfc8b8" }), /* @__PURE__ */ React.createElement("rect", { x: "20", y: "25", width: "40", height: "28", rx: "13", fill: "url(#jbmjdm)", opacity: "0.95" }), /* @__PURE__ */ React.createElement("rect", { x: "22", y: "27", width: "36", height: "24", rx: "11", fill: "url(#jbmvisor)" }), awake ? /* @__PURE__ */ React.createElement("g", { className: "jb-mini-eyes" }, /* @__PURE__ */ React.createElement("circle", { cx: "33", cy: "39", r: "4.4", fill: accent }), /* @__PURE__ */ React.createElement("circle", { cx: "47", cy: "39", r: "4.4", fill: accent }), /* @__PURE__ */ React.createElement("circle", { cx: "31.5", cy: "37.5", r: "1.4", fill: "#fff", opacity: "0.85" }), /* @__PURE__ */ React.createElement("circle", { cx: "45.5", cy: "37.5", r: "1.4", fill: "#fff", opacity: "0.85" })) : /* @__PURE__ */ React.createElement("g", { className: "jb-mini-eyes-closed", stroke: accent, strokeWidth: "2.4", strokeLinecap: "round", opacity: "0.85" }, /* @__PURE__ */ React.createElement("path", { d: "M28.5 39.5q4.5 3 9 0", fill: "none" }), /* @__PURE__ */ React.createElement("path", { d: "M42.5 39.5q4.5 3 9 0", fill: "none" })));
  }
  function readMode() {
    try {
  ...
  const JarvisChatStore = /* @__PURE__ */ function() {
    ...
    async function send(text) {
      ...
      let cfg = {};
      try {
        cfg = window.__JDM_JARVIS_CONFIG__ || JSON.parse(localStorage.getItem("jdm_jarvis_config") || "{}") || {};
      } catch (e) {
        cfg = {};
      }
  ...
  function VizBubble({ viz }) {
    ...
    return /* @__PURE__ */ React.createElement("div", { className: "jb-msg jb-msg--bot jb-viz" }, /* @__PURE__ */ React.createElement("div", { className: "jb-viz-head" }, "🕸️ Sous-graphe : ", /* @__PURE__ */ React.createElement("strong", null, viz.term)), isLive && Live ? /* @__PURE__ */ React.createElement("div", { className: "jb-viz-live" }, React.createElement(Live, {
      term: viz.term,
      nodes: viz.nodes,
      edges: viz.edges || [],
      layout: "tree"
    })) : err ? /* @__PURE__ */ React.createElement("div", { className: "jb-viz-err" }, "⚠️ ", err) : html ? /* @__PURE__ */ React.createElement(
      "iframe",
      {
        title: `viz-${viz.term}`,
        srcDoc: html,
        sandbox: "allow-scripts allow-same-origin",
        className: "jb-viz-frame"
      }
    ) : /* @__PURE__ */ React.createElement("div", { className: "jb-viz-load" }, "… génération du graphe …"));
  }
  function ChatPanel({ dark, onClose }) {
    ...
    const startResize = useCallback((e) => {
      ...
      const onUp = () => {
        ...
        setWidth((w) => {
          try {
            localStorage.setItem("jdm_jarvis_chat_w", String(w));
          } catch (e2) {
          }
          return w;
        });
    ...
    return ReactDOM.createPortal(/* @__PURE__ */ React.createElement(
      "div",
      {
        className: `jb-chat ${dark ? "jb-is-dark" : "jb-is-light"}`,
        role: "dialog",
      ...
      /* @__PURE__ */ React.createElement("div", { className: "jb-chat-head" }, /* @__PURE__ */ React.createElement("span", { className: "jb-chat-bot", "aria-hidden": "true" }, /* @__PURE__ */ React.createElement(MiniRobot, { dark, awake: true })), /* @__PURE__ */ React.createElement("div", { className: "jb-chat-titles" }, /* @__PURE__ */ React.createElement("strong", null, "Jarvis"), /* @__PURE__ */ React.createElement("span", null, "Discussion")), /* @__PURE__ */ React.createElement(
        "button",
        {
          type: "button",
          className: "jb-chat-x",
        ...
        /* @__PURE__ */ React.createElement("svg", { width: "15", height: "15", viewBox: "0 0 16 16", fill: "none" }, /* @__PURE__ */ React.createElement("path", { d: "M2 6V2h4M14 6V2h-4M2 10v4h4M14 10v4h-4", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round", strokeLinejoin: "round" }))
      ), /* @__PURE__ */ React.createElement("button", { type: "button", className: "jb-chat-x", onClick: onClose, title: "Fermer la discussion", "aria-label": "Fermer" }, /* @__PURE__ */ React.createElement("svg", { width: "16", height: "16", viewBox: "0 0 16 16", fill: "none" }, /* @__PURE__ */ React.createElement("path", { d: "M4 4l8 8M12 4l-8 8", stroke: "currentColor", strokeWidth: "1.6", strokeLinecap: "round" })))),
      /* @__PURE__ */ React.createElement("div", { className: "jb-chat-body", ref: scrollRef }, msgs.map((m, i) => m.who === "viz" ? /* @__PURE__ */ React.createElement(VizBubble, { key: i, viz: m.viz }) : m.who === "bot" && m.text ? /* @__PURE__ */ React.createElement(
        "div",
        {
          key: i,
      ...
      ) : /* @__PURE__ */ React.createElement("div", { key: i, className: `jb-msg jb-msg--${m.who}` }, m.text)), busy && /* @__PURE__ */ React.createElement("div", { className: "jb-msg jb-msg--bot jb-msg--typing" }, /* @__PURE__ */ React.createElement("span", null), /* @__PURE__ */ React.createElement("span", null), /* @__PURE__ */ React.createElement("span", null))),
      /* @__PURE__ */ React.createElement("div", { className: "jb-chat-foot" }, /* @__PURE__ */ React.createElement(
        "textarea",
        {
          ref: inputRef,
      ...
      ), /* @__PURE__ */ React.createElement("button", { type: "button", className: "jb-chat-send", onClick: send, disabled: busy || !draft.trim(), title: "Envoyer", "aria-label": "Envoyer" }, /* @__PURE__ */ React.createElement("svg", { width: "16", height: "16", viewBox: "0 0 16 16", fill: "none" }, /* @__PURE__ */ React.createElement("path", { d: "M2.5 8h9M7.5 4l4 4-4 4", stroke: "currentColor", strokeWidth: "1.6", strokeLinecap: "round", strokeLinejoin: "round" }))))
    ), document.body);
  }
  function JarvisBanner() {
    ...
    useEffect(() => JarvisChatStore.subscribe(() => {
      try {
        setChatBusy(!!JarvisChatStore.get().busy);
      } catch (e) {
      }
    }), []);
    ...
    useEffect(() => {
      ...
      return () => {
        window.removeEventListener("__jdm_jarvis_config_changed", sync);
    ...
    const setMode = useCallback((wantAuto) => {
      if (wantAuto) {
        ...
        const cur = readMode();
        const next = cur === "autonome" ? "supervise" : cur;
        writeMode(next);
        setCfgMode(next);
      }
    }, []);
    ...
    if (collapsed) {
      return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { ref: collapsedRef, className: `jb-layer jb-layer--collapsed ${dark ? "jb-is-dark" : "jb-is-light"}` }, /* @__PURE__ */ React.createElement(
        "button",
        {
          type: "button",
          className: `jb-reopen ${auto ? "jb-reopen-awake" : ""}`,
          style: restPos ? { left: restPos.left + "px", top: restPos.top - 21 + "px", right: "auto", bottom: "auto" } : void 0,
        ...
        auto && /* @__PURE__ */ React.createElement("span", { className: "jb-radiate-css", "aria-hidden": "true" }, /* @__PURE__ */ React.createElement("i", null), /* @__PURE__ */ React.createElement("i", null), /* @__PURE__ */ React.createElement("i", null)),
        /* @__PURE__ */ React.createElement(MiniRobot, { dark, awake: auto })
      )), chatOpen && /* @__PURE__ */ React.createElement(ChatPanel, { dark, onClose: () => setChatOpen(false) }));
    }
    return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { className: `jb-layer ${auto ? "jb-mode-auto" : "jb-mode-manual"} ${dark ? "jb-is-dark" : "jb-is-light"}` }, /* @__PURE__ */ React.createElement(
      Robot,
      {
    ...
    ), /* @__PURE__ */ React.createElement("div", { className: "jb-ctl" }, /* @__PURE__ */ React.createElement(
      "div",
      {
        className: "jb-mode-toggle",
        role: "radiogroup",
        "aria-label": "Mode d'exécution",
        style: { background: cardBg, borderColor: line }
      },
      /* @__PURE__ */ React.createElement("span", { className: "jb-mode-thumb", "data-on": auto ? "auto" : "manual" }),
      /* @__PURE__ */ React.createElement(
        "button",
        {
          type: "button",
          className: `jb-mode-btn ${!auto ? "on" : ""}`,
          role: "radio",
          "aria-checked": !auto,
          onClick: () => setMode(false),
          style: { color: !auto ? "#fff" : ink2 }
        },
        /* @__PURE__ */ React.createElement("svg", { width: "13", height: "13", viewBox: "0 0 16 16", fill: "none" }, /* @__PURE__ */ React.createElement("path", { d: "M5 2.5c0-.8.6-1.4 1.3-1.4s1.2.6 1.2 1.4v4.3M7.5 6.8V1.9c0-.8.6-1.3 1.3-1.3s1.2.5 1.2 1.3v4.9M10 7V3c0-.8.5-1.3 1.2-1.3S12.5 2.2 12.5 3v6.2c0 3-2 5.3-4.8 5.3-1.6 0-2.7-.6-3.7-1.9L1.7 9.3c-.4-.6-.3-1.4.3-1.8.5-.4 1.3-.3 1.7.3L5 9.2V3.3c0-.8.5-1.3 1.2-1.3", stroke: "currentColor", strokeWidth: "1.1", strokeLinejoin: "round", strokeLinecap: "round" })),
        "Manuel"
      ),
      /* @__PURE__ */ React.createElement(
        "button",
        ...
        /* @__PURE__ */ React.createElement("svg", { width: "13", height: "13", viewBox: "0 0 16 16", fill: "none" }, /* @__PURE__ */ React.createElement("circle", { cx: "8", cy: "8", r: "2.1", fill: "currentColor" }), /* @__PURE__ */ React.createElement("path", { d: "M8 1.4v1.8M8 12.8v1.8M14.6 8h-1.8M3.2 8H1.4M12.7 3.3l-1.3 1.3M4.6 11.4l-1.3 1.3M12.7 12.7l-1.3-1.3M4.6 4.6L3.3 3.3", stroke: "currentColor", strokeWidth: "1.2", strokeLinecap: "round" })),
        "Autonome"
      )
    ), /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        className: `jb-chat-btn ${chatOpen ? "on" : ""}`,
      ...
      /* @__PURE__ */ React.createElement("svg", { width: "14", height: "14", viewBox: "0 0 16 16", fill: "none" }, /* @__PURE__ */ React.createElement("path", { d: "M2 4.2C2 3 3 2 4.2 2h7.6C13 2 14 3 14 4.2v5.1c0 1.2-1 2.2-2.2 2.2H7l-3.1 2.4c-.4.3-1 0-1-.5v-1.9H4.2C3 11.5 2 10.5 2 9.3V4.2Z", stroke: "currentColor", strokeWidth: "1.2", strokeLinejoin: "round" }), /* @__PURE__ */ React.createElement("circle", { cx: "5.5", cy: "6.75", r: "0.9", fill: "currentColor" }), /* @__PURE__ */ React.createElement("circle", { cx: "8", cy: "6.75", r: "0.9", fill: "currentColor" }), /* @__PURE__ */ React.createElement("circle", { cx: "10.5", cy: "6.75", r: "0.9", fill: "currentColor" })),
      "Discuter"
    ), /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        className: "jb-close",
        title: "Mettre Jarvis en veille",
        onClick: () => setCollapsedPersist(true),
        style: { color: ink3 }
      },
      /* @__PURE__ */ React.createElement("svg", { width: "14", height: "14", viewBox: "0 0 16 16", fill: "none" }, /* @__PURE__ */ React.createElement("path", { d: "M4 4l8 8M12 4l-8 8", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round" }))
    ))), chatOpen && /* @__PURE__ */ React.createElement(ChatPanel, { dark, onClose: () => setChatOpen(false) }));
  }
  const CSS = `
