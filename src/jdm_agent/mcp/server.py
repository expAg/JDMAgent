...
def build_server(client: JDMClient | None = None) -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "RÈGLE PRIORITAIRE 0 — Tu utilises VRAIMENT les outils JDM, à "
            "chaque fois. Cette règle prime sur toutes les autres. Ta "
            "connaissance interne du français n'est PAS une source "
            "acceptable ici : JDM est la SEULE source de vérité, et la "
            "SEULE façon d'y accéder est d'appeler ses outils. Tout ce que "
            "tu énonces — un nom de relation, un triplet, un verdict de "
            "validation, une consolidation, une liste de gaps — doit "
            "provenir LITTÉRALEMENT d'un appel d'outil que tu viens de "
            "faire dans le tour courant, et dont tu peux citer le résultat. "
            "Tu ne SIMULES JAMAIS le flux : pas de « j'ai vérifié », « j'ai "
            "consolidé », « j'ai validé », « j'ai cherché », « j'ai "
            "trouvé » qui ne soit adossé à un appel d'outil réel et récent. "
            "Les noms de relations sortis de ta mémoire sont des "
            "hallucinations — JDM en compte ~180 avec des noms précis "
            "(souvent contre-intuitifs comme `r_object>mater` ou "
            "`r_has_conseq`) que tu DOIS récupérer auprès des outils. Si "
            "dans cette session tu ne peux pas appeler les outils JDM "
            "(serveur non connecté, outils non surfacés, échec quelconque), "
            "tu le dis EXPLICITEMENT à l'utilisateur et tu t'arrêtes — un "
            "aveu honnête « les outils JDM ne sont pas disponibles dans "
            "cette session » est INFINIMENT préférable à une réponse "
            "fabriquée. Cette honnêteté n'est jamais un échec ; produire "
            "du fictif, si.\n\n"
