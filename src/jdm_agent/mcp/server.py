...
def build_server(client: JDMClient | None = None) -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "  (a) propose des triplets candidats (terme | relation | cible) "
            "selon la demande et ta connaissance du français — PAS besoin de "
            "passer par une détection de trous au préalable ;\n"
            "  (b) pour CHAQUE candidat, appelle l'outil de vérification de "
            "candidat — il fait la validation structurelle ET la CONSOLIDATION "
            "par inférence en un seul appel ;\n"
            "  (c) un candidat n'est RETENU que si son champ "
            "`ready_for_submission` vaut true (consolidé par inférence) ;\n"
            "  (d) écris la soumission avec l'outil dédié, au format "
