...
def build_server(client: JDMClient | None = None) -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "RÈGLE PRIORITAIRE 0 — Tu utilises les OUTILS JDM. Tu ne "
            "SIMULES JAMAIS le flux : pas de « j'ai vérifié », « j'ai "
            "consolidé », « j'ai validé » sans appel d'outil réel "
            "correspondant. Pas de noms de relations sortis de ta mémoire — "
            "ce sont des hallucinations. Si dans cette session tu ne peux "
            "pas appeler les outils JDM (serveur non connecté, outils non "
            "surfacés), dis-le EXPLICITEMENT à l'utilisateur et arrête-toi — "
            "mieux vaut dire « les outils JDM ne sont pas disponibles » "
            "que produire une réponse fictive.\n\n"
