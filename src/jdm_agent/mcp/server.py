...
def build_server(client: JDMClient | None = None) -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "8. FLUX D'ENRICHISSEMENT (proposer des triplets pour JDM) : dès "
            "qu'on te demande de PROPOSER / SUGGÉRER des triplets pour combler "
            "des trous de JDM, exécute TOUT le flux toi-même, sans t'arrêter en "
            "chemin et sans qu'on ait à te le redemander :\n"
            "  (a) détecte les trous du terme ;\n"
            "  (b) propose des triplets candidats (terme | relation | cible) ;\n"
            "  (c) pour CHAQUE candidat, appelle l'outil de vérification de "
            "candidat — il fait la validation structurelle ET la CONSOLIDATION "
            "par inférence en un seul appel ;\n"
            "  (d) un candidat n'est RETENU pour la soumission que si son champ "
            "`ready_for_submission` vaut true (consolidé par inférence).\n"
            "La validation structurelle seule (« ok ») NE SUFFIT PAS : ne "
            "déclare JAMAIS un triplet « prêt » ou « validé » sans l'avoir "
            "consolidé. Le fichier / la liste de soumission ne contient QUE les "
            "candidats consolidés, au format `terme|relation|cible|annotation "
            "< explication >` (l'explication = la chaîne d'inférence). Les "
            "non-consolidés sont écartés, pas listés dans la soumission."
        ),
    )
