...
def enrichment_workflow() -> dict:
    ...
    return {
        ...
        "DIVERSITÉ": (
            ...
            "vrai par héritage d'une classe ultra-générique. Exemple de ce "
            "qu'il NE faut PAS proposer :\n"
            "  ✗ tatoueur r_agent-1 marcher / observer / écouter\n"
            "  ✗ potier r_agent-1 méditer / sauter / jeûner\n"
            "  ✗ apiculteur r_agent-1 grimper\n"
            "Ces triplets se consolident trivialement (« personne r_agent-1 "
            "marcher » ⟹ tout métier humain hérite), mais ils n'apportent "
            "RIEN à JDM : ils sont vrais de N'IMPORTE QUEL humain. Ce qui a "
            "de la valeur, c'est ce qui caractérise le métier en propre :\n"
            "  ✓ tatoueur r_agent-1 dessiner / encrer / piquer la peau\n"
            "  ✓ potier r_agent-1 modeler / tourner / cuire l'argile\n"
            "  ✓ apiculteur r_agent-1 enfumer la ruche / récolter le miel\n"
            "Règle simple : si la chaîne de consolidation qu'on te renvoie "
            "passe par une classe générique (personne, individu, être humain, "
            "homme, animal, chose, objet, action…), le triplet est très "
            "probablement trop générique → ne le retiens PAS pour la "
            "soumission, même s'il est « consolidé ». ready_for_submission=true "
            "n'est pas un sauf-conduit ; tu gardes le filtre éditorial.\n\n"
            ...
            "même classe-pivot (personne, animal, …), tu n'as proposé qu'UN "
            "trait générique × N étiquettes — pas N enrichissements distincts. "
            "Choisis d'abord le triplet pour sa pertinence linguistique, "
